...
using Astramentis.Models.PathOfBuilding;
using Discord;
...
namespace Astramentis.Services
{
    ...
    public class Parser
    {
        ...
        private string FromBase64ToXml(string base64)
        {
            ...
            using (var deflate = new ZlibStream(input, (CompressionMode) Ionic.Zlib.CompressionMode.Decompress))
    ...
    public class PasteBinFetcher
    {
        ...
        public async Task<string> GetRawCodeAsync(string url)
        {
            ...
            return await (_endpoint + code).GetStringAsync().ConfigureAwait(false);
        }
    }

    public class PathOfBuildingService
        {
            public async Task<Embed> BuildPathOfBuildingEmbed(string pasteBinURL)
            {
                ...
                embed.AddField("**Info**:", "[Tree](" + character.Tree.Trim() + ")");
